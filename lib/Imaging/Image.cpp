...
void Image::setRGB(int index, rgb colors)
{
    data[index] = colors;
}

void Image::set(int index, int r, int g, int b)
{
    data[index] = rgb(r * max / c, g * max / c, b * max / c);
}

rgb Image::get(int index)
{
    return data[index];
}

rgb Image::getRGB(int h, int w)
{
...
std::string Image::getCabecera()
{
    return cabecera;
}

void Image::setCabecera(std::string c)
{
    cabecera = c;
}
...
std::string Image::toStringRow(int row, const int col_res, const float new_max)
{
    std::string ret = "";
    int index = width * row;
...
Image leer(std::string nombre)
{
    ...
    if (f.is_open())
    {
        ...
        // Busca los dos datos requeridos
        while (numDatos != 2)
        {
            getline(f, linea);
            if (linea[0] != '#')
        ...
        // Una vez leídos los comentarios lee las tuplas de la imagen
...
void escribir(std::string nombre, Image &img, const int col_res)
{
    ...
    if (f.is_open())
    {
        ...
        // Escribe las cabeceras del formato .ppm
        std::string write = "";
